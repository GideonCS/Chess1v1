...
public class PokerSolitaire extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getSource() == newMenuItem) {
			...
			// Creates a StringBuilder with all the high scores in it
			Collections.sort(topPlayers);
			StringBuilder playerData = new StringBuilder();
			for (Player player : topPlayers) {
				playerData.append(player.toString() + "\n");
			}
			// Dialog box for the high score list
	...
	private class DrawingPanel extends JPanel {
		 ...
		 *            the Card to place
		 * @param row
		 *            the row to place the card
		 * @param column
		 *            the column to place the card Precondition row and column
		 *            are on the board
		 */
		...
		private class PokerMouseHandler extends MouseAdapter {
			...
			public void mousePressed(MouseEvent event) {
				...
				// Game is over - check for top scores
					...
					Collections.sort(topPlayers);
					// If there are less than 5 players entered in the
					// Highscores, just add in the player's score and name
					...
					// Otherwise if there are 5 players already and the score is
					// Is better than the last place score, add in the player's
					// Score and name
					else if (score > topPlayers.get(4).getScore()) {
						String name = JOptionPane
								.showInputDialog("Please enter your name ");
						topPlayers.set(4, (new Player(name, score)));
